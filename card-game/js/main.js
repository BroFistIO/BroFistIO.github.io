...
function Game() {
	...
	this.draw = function() {
		...
		// card loading (before the game actually begins) 
		if (!init) {
			jaws.clear();
	...
	this.update = function() {
		...
		// card loading (before the game can be played)
		if (!init) {
			// sound effect timer
			if (timer > 0) {
				if (timer < 1500)  // "start" sound effect length
					...
					/* GAME STARTS HERE */
					timer = 0;
					Game.Spinner.anim[spinner].frame_duration = 100;
					init = true;
				}
				return;
			}
			...
			if (loadCardOffset <= targetOffset) {
				if (++loadCardCount > 9) {
					// finished animating: play sound effect
		...
		// card result
		if (result !== undefined) {
			// card playing (card placing animation is in progress)
	...
	function restart(newHand) {
		...
		switch (Game.settings.AI_OPPONENT) {
			case "RANDOM": opponentAI = new RandomAI(opponentHand, board); break;
			case "OFFENSIVE": opponentAI = new OffensiveAI(opponentHand, board); break;
			case "DEFENSIVE": opponentAI = new DefensiveAI(opponentHand, board); break;
			case "BALANCED":
			default:
				opponentAI = new BalancedAI(opponentHand, board); break;
		}
		switch (Game.settings.AI_PLAYER) {
			case "RANDOM": playerAI = new RandomAI(playerHand, board); break;
			case "OFFENSIVE": playerAI = new OffensiveAI(playerHand, board); break;
			case "DEFENSIVE": playerAI = new DefensiveAI(playerHand, board); break;
			case "BALANCED":
			default:
				playerAI = new BalancedAI(playerHand, board); break;
		}
		result = undefined;
		isCombo = false;
	...
	function playCard(hand, index, position) {
		...
		result = new CardResult(card, position, board);
...
window.onload = function() {
	...
	jaws.preventDefaultKeys([
		"z", "enter", "x", "backspace", "up", "down", "left", "right"]);
